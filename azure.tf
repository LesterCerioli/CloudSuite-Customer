...
resource "azurerm_resource_group" "cloudsolution"{
    name = "cloudsolution"
    location = "East US" = Change the location as per your requirement
}

resource "azurerm_app_service_plan" "container_app_plan"{
    name = "container_app_plan"
...
resource "azurerm_container_registry" "acr"{
    name = "your-container-registry" = Change this to your Azure Container Registry name
    resource_group_name = azurerm_resource_group.cloudsolution.name
    location = azurerm_resource_group.cloudsolution.location
    sku = "Basic"
    admin_enabled = false
...
resource "azurerm_app_service" "container_app"{
    ...
    site_config{
        always_on = true
        linux_fx-version = "DOCKER|<your-acr-name>.azurecr.io/<your-container-image>:<tag>"  # Change this to your ACR and container image details
        app_command_line = ""
        ftps_state = "Disabled"
        use_32_bits_worker_process = false
